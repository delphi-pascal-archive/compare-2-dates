  ...
  with Delta do begin
    ...
    if MM > 0 then                 // si au moins un moins on l'affiche, 'monts' se terminant par un 's' pose moins de problème!
      if texte = '' then
        ...
        texte := format('%2d monts, ', [MM]) + texte;
    if (AA > 0) and
       (Texte <> '') then
      Texte := ', ' + Texte;
    case AA of                    // si au moins un an on l'affiche en l'accordant en nombre
      0 : ;
      1 : texte := '1 year' + Texte;
    else
      texte := format('%2d years', [AA]) + Texte;
    end;
...
begin
  signe := GetYMDBetween(DateTimePicker1.date, DateTimePicker2.date, Years, Months, Days); // appel de la fonction
  // mise en forme du résultat
  texte := '';
  case Days of // si au moins un day on l'affiche en l'accordant en nombre
    0 : ;
    1 : texte := '1 day';
  else
    texte := format('%2d days', [Days]);
  end;
  if Months > 0 then                 // si au moins un moins on l'affiche, 'monts' se terminant par un 's' pose moins de problème!
    if texte = '' then
      texte := format('%2d monts', [Months])
    else
      texte := format('%2d monts, ', [Months]) + texte;
  if (Years > 0) and
     (Texte <> '') then
    Texte := ', ' + Texte;
  case Years of                    // si au moins un an on l'affiche en l'accordant en nombre
    0 : ;
    1 : texte := '1 year' + Texte;
  else
    texte := format('%2d years', [Years]) + Texte;
  end;  
  if not(signe) then // si la 1ère date est postérieure à la seconde
    Label3.Caption := '> ' + Texte
  else
    Label3.Caption := '< ' + Texte;
end;
